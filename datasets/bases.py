...
class ImageTextMaskColorDataset(Dataset):
    ...
    def _build_masked_color_question_and_label(self, caption):
        #代码段的目标是构建一个被部分遮蔽的颜色查询以及相应的标签。这种方法在训练模型理解特定类型的语义信息（如颜色描述）时非常有用，特别是在进行文本到图像或图像到文本的生成任务中。
        #通过这种部分遮蔽和标记的训练数据，模型可以学习到如何准确地理解和生成包含特定颜色信息的文本描述。
